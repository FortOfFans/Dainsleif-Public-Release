...
class Character {
    ...
    constructor(data, uid){
        ...
        for (const [id, value] of Object.entries(data.fightPropMap)){
            if (value === 0 || [1010, 1, 2, 4, 5, 7, 8, 70, 71, 72, 73, 74, 75, 76, 1e3, 1001, 1002, 1003, 1004, 1005,
                1006, 3045, 3046].includes(parseInt(id))) continue;

            this.stats[G[id].replace('FIGHT_PROP_', '')] = value;
        }

        for (const artifact of data.equipList){
            if (artifact.flat.itemType !== 'ITEM_RELIQUARY'){
                this.weapon = artifact.itemId;
                continue;
            }
            this.artifacts.push(new Artifact(artifact, uid, data.avatarId));
        }
    }
}
