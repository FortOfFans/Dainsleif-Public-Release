...
const name = translate('enemy');
const description = translate('Displays an enemy location guide.');
const filter = () => true;
const options = [
    {
        type: 3, //str
        name: translate('name'),
        description: translate('The enemy locations to search for.'),
        required: false
    }
];

async function execute(interaction){
    const basePath = './data/guides/enemy';
    let dirPath = basePath + '/' + interaction.locale;
    if (!fs.existsSync(dirPath)) dirPath = basePath + '/en-GB';
    const enPath = basePath + '/en-GB';

    const aliases = jsonParse('./data/aliases/enemy.json', {});
    ...
    if (!closest || !query){
        const emojiMap = jsonParse('./data/emojis/enemyEmojis.json', {});
        let list = [];
        ...
        const listEmbed = new MessageEmbed()
            ...
            .setDescription(translate('Please select an enemy to view its locations.', interaction.locale));
