...
const name = translate('oculi');
const description = translate('Displays regional oculus.');
const filter = () => true;
const options = [
    {
        type: 3, //str
        name: translate('type'),
        description: translate('The region to search.'),
        required: true,
        choices: [
            {
                name:'Anemoculus',
...
async function execute(interaction){
    const basePath = './data/guides/oculi';
    let dirPath = basePath + '/' + interaction.locale;
    if (!fs.existsSync(dirPath)) dirPath = basePath + '/en-GB';
    const enPath = basePath + '/en-GB';

    const aliases = jsonParse('./data/aliases/oculi.json', {});
    const query = untranslate(interaction.options.getString('type') ?? '', interaction.locale);

    const closest = getClosest((aliases[query] ?? query), fs.readdirSync(enPath).map(f => f.split('.')[0]));

    if (!closest || !query){
        const emojiMap = jsonParse('./data/emojis/oculiEmojis.json', {});
        let list = [];
        ...
        const listEmbed = new MessageEmbed()
            ...
            .setDescription(translate('Please select an Oculi to view its locations.', interaction.locale));
    ...
    const data = jsonParse(dirPath + '/' + untranslate(closest, interaction.locale) + '.json',
        ...
        .map((pg, i, arr) => Object({
            ...
            footer: {
                text: pg.footer.text + ' • '
                    + translate('Page {0} of {0}', interaction.locale)
                        .replace('{0}', i + 1)
